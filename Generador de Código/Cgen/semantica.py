...
def InsertTabla(nombreNodo, aparicion, tipo, arr = None, func = None):
    ...
    if scopeActual == 0 or nodo.arrayObj != None:
        nodo.memLoc = memGlob
        if nodo.arrayObj == None or nodo.arrayObj.tamaño == 0:
            memGlob += 4
        else:
            # print(nodo.arrayObj.tamaño)
            añadir = 4*int(nodo.arrayObj.tamaño)
            # print(añadir)
            memGlob += añadir
    else:
        nodo.memLoc = TablaDeSimbolos[scopeActual].memLocScope
        
        
        TablaDeSimbolos[scopeActual].memLocScope += 4
        for scope in TablaDeSimbolos:
            if scope.funcionDueña == TablaDeSimbolos[scopeActual].funcionDueña:
                scope.memLocScope  = TablaDeSimbolos[scopeActual].memLocScope
    ...
    if tree != None:                        #Mientras el árbol no esté vacío
        ...
        #Si el nodo es un statement
            ...
            if(tree.stmtType == StmtTipo.IF or tree.stmtType == StmtTipo.WHILE):
                newScope = Scope()
                newScope.funcionDueña = TablaDeSimbolos[scopeActual].funcionDueña
                newScope.memLocScope = TablaDeSimbolos[scopeActual].memLocScope
                scopeActual+=1
    ...
    if tree != None:
        ...
        if tree.nType == TipoNodo.EXP:                              #Si el nodo es una expresión
            if tree.expType in [ExpTipo.IDENTIFIER, ExpTipo.CONST]: #Si es un identificador o una constante estableces que es de tipo entero
                ...
                # for scope in range(len(TablaDeSimbolos)):
                #     for nodo in TablaDeSimbolos[scope].nodos:
                #         if nodo.nombreCampo == tree.str and tree.lineno in  nodo.lineasDeAparicion:
                #             ths = nodo
                # if ths.arrayObj != None:
            ...
            #Si es una operación o un assign
                ...
                # if child2.type != child1.type:
                #     errorGenerico("Se deben usar enteros en ambos lados","INT o ARRAY",child2.lineno)
